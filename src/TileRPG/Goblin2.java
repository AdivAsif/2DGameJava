...
public class Goblin2 {
	...
	public Goblin2(float x, float y) throws SlickException {
		timer = new Timer();
		SpriteSheet goblinSheet = new SpriteSheet("res/goblin3.png", 80, 80);
		onetile = 80;
		w = goblinSheet.getSprite(0, 0).getWidth();
		h = goblinSheet.getSprite(0, 0).getHeight();
		pos = new Vector2f(x, y);
		Gob2 = new Animation(goblinSheet, animationSpeed);
	...
	public void silhouette() {
		Gob2.drawFlash(pos.x, pos.y, h, h);
	}
	...
	public void getStats() {
		hp = info.goblin3Attribs[1];
		atk = info.goblin3Attribs[2];
		spd = info.goblin3Attribs[3];
		lck = info.goblin3Attribs[4];
		dfc = info.goblin3Attribs[5];
	}
}